...
class Transformer(t2t_model.T2TModel):
    ...
    def _new_emb_fast_decode(self,
        ...
        if self.has_input:
            ...
            with tf.variable_scope(modality_name):
                bottom = modalities._symbol_bottom_simple_with_emb
                inputs = dp(bottom, inputs, new_emb, hparams)
        ...
        def preprocess_targets(targets, i):
            ...
            with tf.variable_scope(modality_name):
                bottom = modalities._symbol_bottom_simple_with_emb
                targets = dp(bottom, targets, new_emb, hparams)[0]
            targets = common_layers.flatten4d3d(targets)
        ...
        def symbols_to_logits_fn(ids, i, cache):
            ...
            modality_name = hparams.name.get(
                ...
                modalities.get_name(target_modality))(hparams, target_vocab_size)
            with tf.variable_scope(modality_name):
                top = modalities.symbol_top_with_emb
                logits = dp(top, body_outputs, None, new_emb, hparams, target_vocab_size)[0]
    ...
    def _new_emb_infer(self, features, decode_length, new_emb):
        # For real-valued modalities use the slow decode path for now.
        with tf.variable_scope(self.name):
            return self._new_emb_fast_decode(features, new_emb, decode_length)
