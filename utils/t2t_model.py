...
class T2TModel(base.Layer):
  ...
  def _top_single(self, body_output, feature_name, features):
    if self._hparams.loss.get('targets',None) == modalities.generic_sym_l2_loss and self.is_training:
      log_warn("L2 loss, T2TModel.top is a passthrough.")
      return body_output
    if not self._problem_hparams:
  ...
  def extract_embedding(self, features=None):
    set_custom_getter_compose(self._custom_getter)
    with self._eager_var_store.as_default():
      ...
      if self._problem_hparams:
        ...
        with tf.variable_scope(self.name):
          results = self._extract_embedding()
          return results
  ...
  def _extract_embedding(self):
    ...
    if self.has_input:
      ...
      with tf.variable_scope(modality_name):
        bottom = modalities.symbol_bottom_emb_fetch
        emb = dp(bottom, hparams, input_vocab_size)
        return {'emb':emb}
  ...
  def emb_infer(self,
    ...
    with self._eager_var_store.as_default():
      ...
      if beam_size == 1:
        log_info("Greedy Decoding")
        results = self._greedy_emb_infer(features, decode_length, use_tpu)
      else:
  ...
  def infer(self,
    ...
    with self._eager_var_store.as_default():
      ...
      if new_emb is not None:
        log_info("Decoding with new emb")
        results = self._new_emb_infer(features, decode_length, new_emb)
        return results
      if beam_size == 1:
        log_info("Greedy Decoding")
