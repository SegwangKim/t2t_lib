...
def decode_from_dataset(estimator,
  ...
  shard = None if decode_hp.shards < 0 else decode_hp.shard_id
...
def decode_once(estimator,
  ...
  if decode_to_file:
    output_filepath = os.path.join(output_dir, _decode_filename(decode_to_file, problem_name, decode_hp))
    parts = output_filepath.split(".")
    parts[-1] = "targets"
    target_filepath = os.path.join(output_dir, ".".join(parts))
    parts[-1] = "inputs"
    input_filepath = os.path.join(output_dir, ".".join(parts))
