...
def scaling_noise(x, save_noise_to):
  mask = tf.expand_dims(tf.expand_dims(tf.random_uniform(tf.shape(x)[:2]) * 2.0 - 1.0, axis = -1), axis = -1)
  mask = tf.tile(mask, [1,1,1,tf.shape(x)[-1]])
  save_noise_to['noise'] = mask
  return x * cast_like(mask, x)


def scaling_noised(x, save_noise_to, shift = None):
  if shift:
    shifted_noise = shift_right_3d(tf.squeeze(save_noise_to['noise'], axis=2))
    return x * cast_like(shifted_noise, x)
  else:
    return x * cast_like(save_noise_to['noise'], x)
