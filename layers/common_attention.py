...
def encoder_encoder_attention_loss(expected_attention_logits,
                                   actual_attentions,
                                   loss_type="softmax",
                                   loss_multiplier=1.0):
  ...
  def softmax_loss(expected_logits, actual_weights):
    def append_ind(labels):
        batch_size = tf.shape(labels)[0]
        seq_len = tf.shape(labels)[1]
        batch_nums = tf.reshape(tf.tile(tf.range(batch_size), [seq_len]), [-1, seq_len, 1])
        seq = tf.reshape(tf.tile(tf.range(seq_len), [batch_size]), [batch_size, -1, 1])
        int_labels = tf.cast(tf.expand_dims(labels, -1), dtype=tf.int32) 
        labels_ind = tf.concat([batch_nums, seq, int_labels], axis=-1)
        return labels_ind
    #shape = tf.shape(actual_weights)
    #reshape_logit = tf.reshape(expected_logits, shape = [shape[0], shape[1], shape[2]])
    ...
    #print('reshape_logit')
    #print(reshape_logit)
    expected_logits_ind = append_ind(expected_logits)
    #expected_logits_ind = tf.identity(expected_logits_ind, "BUS2")
    #print(actual_weights)
    #actual_weights = tf.identity(actual_weights, "BUS")
    return tf.reduce_mean(-tf.log(tf.gather_nd(actual_weights, expected_logits_ind)+0.001))
  ...
  if loss_type == "softmax":
    actual_encdec_attention_logits = [
        ...
        if "self_attention" in layer_key and layer_key.endswith("dot_product_attention")
    ]
    actual_attention_logits = combine_attentions(actual_encdec_attention_logits)
    loss = softmax_loss(expected_attention_logits,
...
def encoder_decoder_attention_loss(expected_attention_logits,
  ...
  # def softmax_loss_with_logits(expected_logits, actual_weights):
  #   int_labels = tf.cast(expected_logits, tf.int32)
  #   return tf.losses.sparse_softmax_cross_entropy(labels = int_labels, logits = actual_weights)
  ...
  def softmax_loss(expected_logits, actual_weights):
    ...
    expected_logits_ind = append_ind(expected_logits)
    #print(actual_weights)
    return tf.reduce_mean(-tf.log(tf.gather_nd(actual_weights, expected_logits_ind)+0.000000001))
  # For each hidden layer, we have attention-logit and attention-weight tensors
  # with shape [batch_size, num_heads, target_length, input_length].
  loss = 0.0
  if loss_type == "mse":
    ...
    actual_encdec_attention_logits = [
        t for layer_key, t in actual_attentions.items()
        if "encdec_attention" in layer_key and layer_key.endswith("dot_product_attention")
    ]
    actual_attention_logits = combine_attentions(actual_encdec_attention_logits)
    loss = softmax_loss(expected_attention_logits,
                              actual_attention_logits)
  # elif loss_type == "softmax_with_logits":
  #   actual_encdec_attention_logits = [
  #       t for layer_key, t in actual_attentions.items()
  #       if "encdec_attention" in layer_key and layer_key.endswith("/logits")
  #   ]
  #   actual_attention_logits = combine_attentions(actual_encdec_attention_logits)
  #   loss = softmax_loss_with_logits(expected_attention_logits,
  #                             actual_attention_logits)
  else: # KL divergence
    actual_encdec_attention_logits = [
        t for layer_key, t in actual_attentions.items()
