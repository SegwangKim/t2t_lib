...
def _symbol_bottom_emb_fetch_simple(model_hparams, vocab_size, name, reuse):
    """Bottom transformation for symbols."""
    with tf.variable_scope(name, reuse=reuse):
        var = get_weights(model_hparams, vocab_size)
        return var
...
def symbol_bottom_emb_fetch(model_hparams, vocab_size):
    if (model_hparams.shared_embedding_and_softmax_weights or
            model_hparams.get("shared_embedding")):
        return _symbol_bottom_emb_fetch_simple(model_hparams, vocab_size, "shared", reuse=None)


def _symbol_bottom_simple_with_emb(x, emb, model_hparams):
    # Ensure the inputs are 3-D
    if len(x.get_shape()) == 4:
        x = tf.squeeze(x, axis=3)
    while len(x.get_shape()) < 3:
...
def generic_sym_l2_loss(body_output,
                    targets,
                    model_hparams,
                    vocab_size,
                    weights_fn):
    name = get_name('symbol')(model_hparams, vocab_size)
    with tf.variable_scope(name) as tm_vs:
        emb_targets = symbol_bottom(targets, model_hparams, vocab_size)
    if tf.shape(body_output) != tf.shape(emb_targets):
        return generic_loss(body_output, targets, model_hparams, vocab_size, weights_fn)
    loss = tf.squared_difference(body_output, tf.to_float(emb_targets))
...
@is_pointwise
def symbol_top_with_emb(body_output, targets, emb, model_hparams, vocab_size):
    """Generate logits.
